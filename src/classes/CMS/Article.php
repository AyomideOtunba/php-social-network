...
class Article
{
    ...
    {
        $arguments['category'] = $arguments['category1'] = $category;  // Category id
    ...
    public function update(array $article, string $temporary, string $destination): bool
    {
        try {                                            // Try to update data
            ...
            unset($article['category'], $article['seo_category'], $article['created'], $article['forename'], $article['surname'], $article['author'], $article['image_file'], $article['image_alt'], $article['likes'], $article['comments']);
            $sql = "UPDATE article 
